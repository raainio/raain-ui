...
function getLatestGitTag() {
  try {
    ...
    const tag = execSync('git tag -l "v*.*.*" --sort=-v:refname | head -n 1', { stdio: 'pipe' }).toString().trim();
...
function getGitTagDate(tag) {
  try {
    ...
    const dateStr = execSync(`git log -1 --format=%ad --date=short v${tag}`, { stdio: 'pipe' }).toString().trim();
