...
export class MapTools {
    static getPolarDistanceRatio(
        centerPoint: Point,
        lastGeoValue: PolarMapValue,
        polarMap2Point: (pv: PolarMapValue) => Point
    ): number {
    ...
    static getCartesianDistanceRatio(
        center: MapLatLng,
    ...
    static hexStringToNumber(hexString: string): number {
        const hex =
            hexString.substring(1, 3) + hexString.substring(3, 5) + hexString.substring(5, 7);
