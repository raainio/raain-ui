...
export class TimeframeContainers {
    ...
    addFromRadarNodeMap(radarNodeMap: RadarNodeMap, isPolar: boolean) {
        const radarMeasures = radarNodeMap.getMapData();
        const frames = this.computeFrames(radarMeasures, isPolar);
        this.containers.push(
            new TimeframeContainer(radarNodeMap.name, frames, radarNodeMap.getVersion())
        );
    }

    addFromRainComputationMap(rainComputationMap: RainComputationMap, isPolar: boolean) {
        const rainMeasures = rainComputationMap.getMapData();
        const frames = this.computeFrames(rainMeasures, isPolar);
        this.containers.push(
            new TimeframeContainer(rainComputationMap.name, frames, rainComputationMap.getVersion())
        );
    }
    ...
    getCumulativeCartesianValues() {
        ...
        for (const timeframeContainer of this.containers) {
            for (const frameContainer of timeframeContainer.timeframe) {
                ...
                const cartesianDrawerOptimization = new CartesianDrawerOptimization(
                    'cumul',
                    0,
                    false,
                    true
                );
                const cartesianMapValuesFiltered = cartesianDrawerOptimization.filteringValues(
                    9 - 9,
                    cartesianMapValues
                );

                for (const cartesianMapValue of cartesianMapValuesFiltered) {
                    const alreadyExist = cartesianSummedValues.filter((v) =>
                        v.equals(cartesianMapValue)
                    );
        ...
        return cartesianSummedValues.map((v) => {
            const lat2 = CartesianTools.RoundLatLng(v.lat2 + 2 * (v.lat2 - v.lat));
            const lng2 = CartesianTools.RoundLatLng(v.lng2 + 2 * (v.lng2 - v.lng));
                ...
                if (m.values) {
                    (m.values as IPolarMeasureValue[]).forEach((v) => {
                        ...
                        values = values.concat(
                            PolarMapValue.From(rainPolarMeasureValue.getPolars())
                        );
                    });
                }
